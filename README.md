...
`create-invite` and `accept-invite` read `APP_URL` (the deployed web app's origin, e.g. `https://guardian.example.com`) from Supabase project secrets — the same secret already used by `send-alert`. `push-test` additionally needs the VAPID secrets described in **Web Push setup** below:
...
Guardian can send real Web Push notifications (free — no FCM/APNs account needed) via VAPID. This app ships with no service worker or manifest by default in git; both were added under `public/` for this feature.
