...
A premium, mobile-first personal safety app that allows users to activate discreet safety sessions with natural-sounding Sotto Phrases.

## Features

- **Sotto Mode Sessions**: Activate temporary safety sessions (30 min, 1 hour, 2 hours, or manual)
- **Sotto Phrases**: Create natural-sounding phrases that trigger private responses
...
`create-invite` and `accept-invite` read `APP_URL` (the deployed web app's origin, e.g. `https://sotto.example.com`) from Supabase project secrets — the same secret already used by `send-alert`. `push-test` additionally needs the VAPID secrets described in **Web Push setup** below:
...
| `/app/start-guard-mode` | Configure and start Sotto Mode |
| `/app/guard-mode-active` | Active session screen |
| `/app/safe-phrases` | Sotto Phrase builder |
...
- Sotto does not directly contact emergency services
...
**Copy honesty:** there is no native app or app-store listing yet, so invite copy always says "Open Sotto," never "Download the app." Once a Capacitor build and a registered universal-link domain association exist, `/join` links can be updated to deep-link into the installed app first and fall back to the app-store listing. Today they simply open the web app in the visitor's browser and auto-fill the invite code/token.

### Web Push setup

Sotto can send real Web Push notifications (free — no FCM/APNs account needed) via VAPID. This app ships with no service worker or manifest by default in git; both were added under `public/` for this feature.
